...
public class GenTest : MonoBehaviour
{
	...
	void Start()
	{
		transformCoord = transform.position;

		material.SetVector("_center", transformCoord);
		InitTextures();
		CreateBuffers();
	...
	void ComputeDensity()
	{
		...
		densityCompute.SetVector("center", transformCoord);
	...
	void GenerateChunk(Chunk chunk)
	{
		...
		meshCompute.SetVector("transformCoord", transformCoord);
	...
	public void Terraform(Vector3 point, float weight, float radius)
	{
		...
		//Coordinates of terraforming
		float tx = Mathf.Clamp01((point.x - transformCoord.x + boundsSize / 2) / boundsSize);
		float ty = Mathf.Clamp01((point.y - transformCoord.y + boundsSize / 2) / boundsSize);
		float tz = Mathf.Clamp01((point.z - transformCoord.z + boundsSize / 2) / boundsSize);
		...
		Debug.Log(new Vector3Int(editX, editY, editZ));

		editCompute.SetFloat("weight", weight);
		editCompute.SetFloat("deltaTime", Time.deltaTime);
		editCompute.SetVector("transformCoord", transformCoord);
		...
		for (int i = 0; i < chunks.Length; i++)
		{
			...
			if (MathUtility.SphereIntersectsBox(point, worldRadius, chunk.centre + transformCoord, Vector3.one * chunk.size))
			{
				//Debug.Log(chunk.centre + transformCoord);
				chunk.terra = true;
				GenerateChunk(chunk);
