...
public class Terraformer : MonoBehaviour
{
	...
	void Update()
	{
		...
		for (int i = 0; i < numIterations; i++)
		{
			...
			if (Physics.SphereCast(cam.position, rayRadius, cam.forward, out hit, 1000, terrainMask, QueryTriggerInteraction.Ignore))
			{
	...
	void Terraform(Vector3 terraformPoint)
	{
		...
		if (Input.GetMouseButton(0))
		{
			...
			pCont.NotifyTerrainChanged(terraformPoint, terraformRadius);
		}
		// Subtract terrain
